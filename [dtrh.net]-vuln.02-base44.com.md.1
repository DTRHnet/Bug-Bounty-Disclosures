...
6. **Reconsider using the same `APP_ID` as both a path-scoping value and an implicit authorization signal** in the file-serving endpoint. Given the July 2025 `app_id`-trust incident, any endpoint that accepts `APP_ID` in the URL should explicitly re-verify the *requester's* authorization to that application, rather than treating a correctly-shaped path as sufficient.
7. **Confirm and document `REF_ID` generation strength.** If `REF_ID` is meant to function as an unguessable capability token (bearer-URL security model), publish that as an explicit design decision, ensure sufficient entropy and no sequential/time-based leakage, and ensure it is never logged, referrer-leaked, or exposed in client-side code alongside the `APP_ID` it pairs with. If it is not meant to function this way, authentication must not be optional at the serving layer.
